...
function addRootElement(rootElem: Element): void {
  const { nextElementSibling } =
    document.body.lastElementChild || ({ nextElementSibling: null } as Element);
  document.body.insertBefore(rootElem, nextElementSibling);
}

function checkIsRemovePropertyExisted(element: Element): boolean {
...
function usePortal(id: string): Element {
  ...
  useEffect(() => {
    ...
    return function removeElement() {
      ...
      if (
        parentElem.childNodes.length === 0 &&
