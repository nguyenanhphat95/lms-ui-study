...
class Calendar extends PureComponent {
    ...
    updateShownDate(props = this.props) {
        const newProps = props.scroll.enabled
            ? Object.assign(Object.assign({}, props), { months: this.list.getVisibleRange().length }) : props;
    ...
    renderMonthAndYear(focusedDate, changeShownDate, props) {
        ...
        return (<Grid container spacing={2} onMouseUp={(e) => e.stopPropagation()} className={styles.monthAndYearWrapper}>
        <Grid item xs={6} container justifyContent="space-between" alignItem="center">
          {showMonthArrow && (<Button fullWidth={false} color={ButtonColors.secondary} buttonSize={ButtonSizes.sm} icon={ChevronLeft} onClick={() => changeShownDate(-1, "monthOffset")}/>)}

          <DropdownList value={focusedDate.getMonth()} onChange={(value) => changeShownDate(value, "setMonth")} size={InputSizes.sm} className={styles.monthAndYearSelect} menuClassName={styles.monthAndYearSelectMenu}>
            {new Array(12).fill(null).map((_, month) => {
            const monthName = month + 1;
            return (<Option key={month} value={month} size={InputSizes.sm}>
                  {`Tháng ${monthName}`}
                </Option>);
        })}
          </DropdownList>
        ...
        <Grid item xs={6} container justifyContent="space-between" alignItem="center">
          <DropdownList value={focusedDate.getFullYear()} onChange={(value) => changeShownDate(value, "setYear")} size={InputSizes.sm} className={styles.monthAndYearSelect} menuClassName={styles.monthAndYearSelectMenu}>
            {new Array(upperYearLimit - lowerYearLimit + 1)
            .fill(upperYearLimit)
            .map((val, i) => {
            const year = val - i;
            return (<Option key={year} value={year} size={InputSizes.sm}>
                    {`Năm ${year}`}
                  </Option>);
        })}
          </DropdownList>
          {showMonthArrow && (<Button fullWidth={false} color={ButtonColors.secondary} buttonSize={ButtonSizes.sm} icon={ChevronRight} onClick={() => changeShownDate(+1, "monthOffset")}/>)}
        </Grid>
      </Grid>);
    }
    ...
    renderHour(type) {
        ...
        const dateSelected = type === "startHour" ? ranges[0].startDate : ranges[0].endDate;
        const isDisabled = (type === "startHour" && !ranges[0].startDate) ||
            (type === "endHour" && !ranges[0].endDate);
        const hourFormatted = this.getTimeFromDate("hour", dateSelected);
        return (<DropdownList className={styles.hourSelect} name={type} value={hourFormatted} disabled={isDisabled} onChange={(value) => this.onChangeTime(type, value)} size={InputSizes.xs} menuClassName={styles.hourAndMinuteSelectMenu}>
        {new Array(24).fill(null).map((_, hour) => {
            const hourText = hour < 10 ? `0${hour}` : `${hour}`;
            return (<Option key={hour} value={hour} size={InputSizes.xs}>
              {hourText}
            </Option>);
        })}
      </DropdownList>);
    ...
    renderMinute(type) {
        const { ranges } = this.props;
        const dateSelected = type === "startMinute" ? ranges[0].startDate : ranges[0].endDate;
        const isDisabled = (type === "startMinute" && !ranges[0].startDate) ||
            (type === "endMinute" && !ranges[0].endDate);
        const minuteFormatted = this.getTimeFromDate("minute", dateSelected);
        return (<DropdownList className={styles.hourSelect} name={type} value={minuteFormatted} disabled={isDisabled} onChange={(value) => this.onChangeTime(type, value)} size={InputSizes.xs} menuClassName={styles.hourAndMinuteSelectMenu}>
        {new Array(60).fill(null).map((_, minute) => {
            const minuteText = minute < 10 ? `0${minute}` : `${minute}`;
            return (<Option key={minute} value={minute} size={InputSizes.xs}>
              {minuteText}
            </Option>);
        })}
      </DropdownList>);
    ...
    renderConfirmDate() {
        const { hasTime, dateDisplayFormat, date, ranges, months, onResetFocusRange, onCancel, onConfirm, rangeSelected, } = this.props;
        const formatPattern = hasTime ? dateDisplayFormat : "dd/MM/yyyy";
        const isRangeNotNull = ranges.length > 0 &&
            (isValid(ranges[0].startDate) || isValid(ranges[0].endDate));
        const isRangeValid = (ranges.length > 0 &&
            isValid(ranges[0].startDate) &&
            isValid(ranges[0].endDate)) ||
            isValid(date);
        let timeFormatted = "";
        if (isRangeNotNull) {
            const startTimeFormatted = this.formatDateDisplay(ranges[0].startDate, "", formatPattern);
            const endTimeFormatted = this.formatDateDisplay(ranges[0].endDate, "", formatPattern);
        ...
        const showResetButton = (!!rangeSelected &&
            isValid(rangeSelected.startDate) &&
            isValid(rangeSelected.endDate)) ||
            isValid(date);
        return (<Grid container spacing={8} direction="column">
        <Grid item container justifyContent="space-between" className={cn(styles.confirmDate, {
            [styles.hasOneMonth]: months === 1,
        })}>
          <div>
            {showResetButton && (<Button className={styles.btnReset} icon={RotateCcw} buttonSize={ButtonSizes.sm} color={ButtonColors.secondary} onClick={this.handleResetDate}/>)}

            {months > 1 && (<span className={styles.dateRangeResult}>{timeFormatted}</span>)}
          </div>
          <div>
            <Button color={ButtonColors.secondary} buttonSize={ButtonSizes.sm} className={styles.btnConfirm} onClick={handleCancel}>
              Thoát
            </Button>
            <Button color={ButtonColors.primary} buttonSize={ButtonSizes.sm} className={styles.btnConfirm} disabled={!isRangeValid && !this.state.isReset} onClick={onConfirm}>
              Áp dụng
            </Button>
          </div>
        </Grid>
    ...
    render() {
        const { onPreviewChange, direction, disabledDates, rangeColors, color, months, } = this.props;
        const { focusedDate } = this.state;
        const isVertical = direction === "vertical";
        const navigatorRenderer = this.props.navigatorRenderer || this.renderMonthAndYear;
        const ranges = this.props.ranges.map((range, i) => (Object.assign(Object.assign({}, range), { color: range.color || rangeColors[i] || color })));
        const _focusedDate = isValid(focusedDate)
            ? focusedDate
            : subMonths(new Date(), 1);
        return (<div className={cn(styles.calendarWrapper, this.props.className)} onMouseUp={() => this.setState({ drag: { status: false, range: {} } })} onMouseLeave={() => {
            this.setState({ drag: { status: false, range: {} } });
        }}>
        <Grid container direction="column" spacing={4}>
          ...
          <Grid item className={cn(styles.months, isVertical ? styles.monthsVertical : styles.monthsHorizontal)}>
            {new Array(months).fill(null).map((_, i) => {
            const monthStep = addMonths(_focusedDate, i);
            return (<Month {...this.props} onPreviewChange={this.props.onPreviewChange || this.updatePreview} preview={this.props.preview || this.state.preview} ranges={ranges} key={i} drag={this.state.drag} dateOptions={this.dateOptions} disabledDates={disabledDates} month={monthStep} onDragSelectionStart={this.onDragSelectionStart} onDragSelectionEnd={this.onDragSelectionEnd} onDragSelectionMove={this.onDragSelectionMove} onMouseLeave={() => onPreviewChange()} styles={styles} showWeekDays={!isVertical || i === 0} showMonthName={months > 1}/>);
        })}
          </Grid>

          {this.props.hasTime && (<Grid item className={styles.timeDisplay}>
              <div className={styles.timeItem}>
                {this.renderHour("startHour")}
                <span className={styles.timeSeparate}>:</span>
                {this.renderMinute("startMinute")}
              </div>
              <ArrowRight width="12px" height="24px" className={styles.arrowSeparateTime}/>
              <div className={styles.timeItem}>
