...
const staticRangeHandler = {
    ...
    isSelected(range) {
        ...
        return (isSameDay(range.startDate, definedRange.startDate) &&
            isSameDay(range.endDate, definedRange.endDate));
    },
    isDisabled(minDate, maxDate) {
        const currentRange = this.range();
        return (isBefore(currentRange.startDate, minDate) ||
            isAfter(currentRange.endDate, maxDate));
    },
};
export function createStaticRanges(ranges) {
    return ranges.map((range) => (Object.assign(Object.assign({}, staticRangeHandler), range)));
}
...
export const defaultInputRanges = [
    {
        ...
        range(value, { minDate }) {
            ...
            const startDate = addDays(defineds.startOfToday, (Math.max(Number(value), 1) - 1) * -1);
        ...
        isDisabled(minDate, maxDate) {
            return (isAfter(minDate, startOfDay(new Date())) ||
    ...
    },
    {
        ...
        isDisabled(minDate, maxDate) {
            return (isBefore(maxDate, endOfDay(new Date())) ||
