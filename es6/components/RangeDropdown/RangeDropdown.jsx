...
const RangeDropdown = (props) => {
    const _a = Object.assign(Object.assign({}, defaultProps), props), { onClose, onChange, className, iconClassName, disabledInput = true, placeholder, disabled, recommendData, unit, defaultValues, inline, placeholderFrom, placeholderTo, billionLabel, millionLabel, title, deleteLabel } = _a, rest = __rest(_a, ["onClose", "onChange", "className", "iconClassName", "disabledInput", "placeholder", "disabled", "recommendData", "unit", "defaultValues", "inline", "placeholderFrom", "placeholderTo", "billionLabel", "millionLabel", "title", "deleteLabel"]);
    ...
    const classNameOfIcons = cn(iconClassName, styles["dropdown-icon"], styles[`dropdown-addon-size-${rest.size}`], {
        [styles["icon-open"]]: isOpen,
    });
    const afterInput = useMemo(() => (<InputAdornment>
        <Icon onClick={_showDropdown} className={classNameOfIcons} component={ChevronRight}/>
      </InputAdornment>), [isOpen, disabled]);
    const _handleSelectPriceRange = (item) => {
        setValue(item.id);
    ...
    const _handleChangePriceFrom = (e) => {
        setValue(null);
    ...
    let display = (<Typography type={TypoTypes.sub}>{placeholder}</Typography>);
    if (value !== null && typeof value !== "undefined") {
        const data = _find(recommendData, (x) => x.id === value);
        if (data) {
            display = (<Typography type={TypoTypes.sub}>{_get(data, "text")}</Typography>);
        }
    }
    ...
    const content = (<Grid container direction="column" spacing={3}>
      ...
      <Grid item>
        <Box pb={1}>
          <Grid container spacing={2} alignItem="center" nowrap justifyContent="space-between">
            <Grid item xs={true}>
              <NumberFormat placeholder={placeholderFrom} min={0} max={1000000000000} size={inputSize} value={_getCurrentPriceFrom()} onChange={_handleChangePriceFrom} decimalSeparator={false}/>
            </Grid>
            <Grid item xs="auto">
            ...
            <Grid item xs={true}>
              <NumberFormat placeholder={placeholderTo} min={0} max={1000000000000} size={inputSize} value={_getCurrentPriceTo()} onChange={_handleChangePriceTo} decimalSeparator={false}/>
            </Grid>
          </Grid>
        </Box>
      </Grid>
      ...
      <Grid item>
        ...
        return (<Grid item xs={col} key={index}>
                <Chip component="div" key={index} className={cn(styles["chip"], {
            [styles.active]: value === item.id,
        })} label={item.text} onClick={() => _handleSelectPriceRange(item)}/>
              </Grid>);
    })}
        </Grid>
      </Grid>
    ...
    return (<div ref={anchorRef} className={classOfRoot}>
      <Input className={cn(styles.input, { [styles["is-open"]]: isOpen })} onClick={_showDropdown} onClickAfterInput={_showDropdown} component={PriceRangeInput} afterInput={afterInput} readOnly={disabledInput} display={display} disabled={disabled} {...rest}/>
      {isOpen && (<Popover anchorRef={anchorRef} backdrop={BackdropTypes.transparent} placement={PopperPlacements.bottomStart} onClose={_hideDropdown} className={styles.popover}>
          <Box p={4}>{content}</Box>
        </Popover>)}
    </div>);
};
